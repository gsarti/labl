...
def unsupervised_qe_metrics_fn(
    ...
    all_layer_states = get_decoder_states(args.forward_output)
...
@torch.no_grad()
def unsupervised_qe_metrics_light_fn(args: StepFunctionArgs) -> SingleScorePerStepTensor:
    all_layer_states = get_decoder_states(args.forward_output)
    out_logits = args.attribution_model.output2logits(args.forward_output)
    output_logprobs = F.log_softmax(out_logits, dim=-1)
    logprob = output_logprobs[0, args.target_ids].item()
...
def get_metric_names(model: AttributionModel, is_heavy: bool = True) -> list[str]:
